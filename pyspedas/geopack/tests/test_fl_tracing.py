...
class LoadTestCases(unittest.TestCase):
    ...
    def test_t89_iono_n_idl_single(self):
        ...
        store_data('py_trace_n',data={'x':py_time_array, 'y':trace_points})
        pyspedas.set_coords('py_trace_n','gsm')
        pyspedas.set_units('py_trace_n','km')
        store_data('idl_trace_n',data={'x':idl_time_array, 'y':idl_tp_array})
        ...
        tplotxy3(['py_trace_n', 'idl_trace_n'], legend_names=['py','idl'], colors=['black', 'red'], markers=['+', '>'], reverse_x=True,plot_units='km',show_centerbody=False, display=True)

    def test_t89_iono_s_idl_single(self):
        d1=get_data('tha_state_pos')
        ...
        t = d1.times[0]
        pos = d1.y[0,:]
        trace_points, foot_point, sol = trace_iono_89(t, pos, km=True, iopt=3, south=True)
        n_py_trace = len(trace_points)
        n_idl_trace = d2.y.shape[1]
        py_tp_array = np.zeros((n_py_trace,3))
        py_tp_array[:,:] = trace_points
        ...
        store_data('py_trace_s',data={'x':py_time_array, 'y':trace_points})
        pyspedas.set_coords('py_trace_s','gsm')
        pyspedas.set_units('py_trace_s','km')
        store_data('idl_trace_s',data={'x':idl_time_array, 'y':idl_tp_array})
        print(symmetric_trace_distance(py_tp_array, idl_tp_array))
        print(directed_trace_distance_with_worst_point(py_tp_array,idl_tp_array))
        print(f"py foot point {foot_point}, idl foot point {d3.y[0,:]}")
        print(f"foot point distance: {np.linalg.norm(foot_point-d3.y[0,:])}")
        print(f"foot point r: python {np.linalg.norm(foot_point)} idl: {np.linalg.norm(d3.y[0,:])}")
        #tplotxy(['py_trace'], plane='xy')
        tplotxy3(['py_trace_s', 'idl_trace_s'], legend_names=['py','idl'], colors=['black', 'red'], markers=['+', '>'], reverse_x=True,plot_units='km',show_centerbody=False, display=True)
    ...
    def test_t89_iono_n_idl_all(self):
        ...
        #tplotxy3(['tha_iono_t89_trace'])
        d3=get_data('ifoot_t89_n')
        ttrace2iono_89('tha_state_pos','py_foot_n', 'py_trace_n', iopt=3, km=True)
    ...
    def test_t89_iono_s_idl_all(self):
        d1=get_data('tha_state_pos')
        d2=get_data('tha_iono_t89_trace_s')
        #tplotxy3(['tha_iono_t89_trace'])
        d3=get_data('ifoot_t89_s')
        ttrace2iono_89('tha_state_pos','py_foot_s', 'py_trace_s', iopt=3, km=True, south=True)
