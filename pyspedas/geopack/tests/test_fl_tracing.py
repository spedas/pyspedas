...
class LoadTestCases(unittest.TestCase):
    ...
    def test_t89_iono_n_idl_single(self):
        d1=get_data('tha_state_pos')
        d2=get_data('tha_iono_t89_trace')
        #tplotxy3(['tha_iono_t89_trace'])
        d3=get_data('ifoot_t89')
        t = d1.times[0]
        pos = d1.y[0,:]
        trace_points, foot_point, sol = trace_iono_89(t, pos, km=True, iopt=3)
        ...
        store_data('py_trace',data={'x':py_time_array, 'y':trace_points})
        pyspedas.set_coords('py_trace','gsm')
        pyspedas.set_units('py_trace','km')
        store_data('idl_trace',data={'x':idl_time_array, 'y':idl_tp_array})
        ...
        tplotxy3(['py_trace', 'idl_trace'], legend_names=['py','idl'], colors=['black', 'red'], markers=['+', '>'], reverse_x=True,plot_units='km',show_centerbody=False, display=True)

    def test_t89_iono_n_idl_all(self):
        d1=get_data('tha_state_pos')
        d2=get_data('tha_iono_t89_trace')
        #tplotxy3(['tha_iono_t89_trace'])
        d3=get_data('ifoot_t89')
        ttrace2iono_89('tha_state_pos','py_foot', 'py_trace', iopt=3, km=True)
        py_trace_data = get_data('py_trace')
        py_foot_data = get_data('py_foot')
        idl_trace_data = d2.y
        idl_foot_data = d3.y

        set_coords('py_foot','GSM')
        set_units('py_foot','km')
        set_coords('py_trace','GSM')
        set_units('py_trace','km')
        ...
        store_data('py_trace1',data={'x':py_time_array, 'y':trace_points})
        pyspedas.set_coords('py_trace1','gsm')
        pyspedas.set_units('py_trace1','km')
        store_data('idl_trace1',data={'x':idl_time_array, 'y':idl_tp_array})
        pyspedas.set_coords('idl_trace1','gsm')
        pyspedas.set_units('idl_trace1','km')
        ...
        print(f"foot point r: python {np.linalg.norm(foot_point)} idl: {np.linalg.norm(d3.y[0,:])}")
        #tplotxy(['py_trace'], plane='xy')
        tplotxy3(['py_trace1', 'idl_trace1'], legend_names=['py','idl'], colors=['black', 'red'], markers=['+','>'], reverse_x=True,plot_units='km',show_centerbody=False, display=True)
        tplotxy3(['py_trace', 'tha_iono_t89_trace', 'tha_state_pos'], legend_names=['py','idl','orbit'], colors=['black', 'red','blue'], markers=[None, None, '+'], reverse_x=True,plot_units='km',show_centerbody=False, display=True)
        tplotxy3(['py_foot', 'ifoot_t89'], legend_names=['py','idl'], colors=['green','red'],markers=['+',None], reverse_x=True, plot_units = 'km',show_centerbody=True, display=True)

    def test_t89_equ_single(self):
        d1=get_data('tha_state_pos')
        ...
        t = d1.times[0]
        pos = d1.y[0,:]
        trace_points, foot_point, sol = trace_equator_89(t, pos, km=True, iopt=3)
        n_py_trace = len(trace_points)
        n_idl_trace = d2.y.shape[1]
        py_tp_array = np.zeros((n_py_trace,3))
        py_tp_array[:,:] = trace_points
        ...
        store_data('py_equ_trace',data={'x':py_time_array, 'y':trace_points})
        pyspedas.set_coords('py_equ_trace','gsm')
        pyspedas.set_units('py_equ_trace','km')
        store_data('idl_equ_trace',data={'x':idl_time_array, 'y':idl_tp_array})
        pyspedas.set_coords('idl_equ_trace','gsm')
        pyspedas.set_units('idl_equ_trace','km')
        print(symmetric_trace_distance(py_tp_array, idl_tp_array))
        print(directed_trace_distance_with_worst_point(py_tp_array,idl_tp_array))
        print(f"py foot point {foot_point}, idl foot point {d3.y[0,:]}")
        print(f"foot point distance: {np.linalg.norm(foot_point-d3.y[0,:])}")
        print(f"foot point r: python {np.linalg.norm(foot_point)} idl: {np.linalg.norm(d3.y[0,:])}")
        #tplotxy(['py_trace'], plane='xy')
        tplotxy3(['py_equ_trace', 'idl_equ_trace'], legend_names=['py','idl'], colors=['black', 'red'], markers=['+','>'], reverse_x=True,plot_units='km',show_centerbody=True, display=True)

    def test_t89_equ_n_idl_all(self):
        d1=get_data('tha_state_pos')
        d2=get_data('tha_eq_t89_trace')
        #tplotxy3(['tha_iono_t89_trace'])
        d3=get_data('eq_foot_t89')
        ttrace2equator_89('tha_state_pos','py_eq_foot', 'py_eq_trace', iopt=3, km=True)
