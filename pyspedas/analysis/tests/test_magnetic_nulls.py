...
class MagNullTestCases(unittest.TestCase):
    ...
    def test_find_magnetic_nulls_fote_mms(self):
        data = pyspedas.projects.mms.fgm(
            probe=[1, 2, 3, 4],
            trange=["2016-09-19/07:40", "2016-09-19/07:45"],
            data_rate="srvy",
            time_clip=True,
            varformat="*_gse_*",
        ...
        null_vars = pyspedas.find_magnetic_nulls_fote(
            fields=fields,
            positions=positions,
            smooth_fields=True,
            smooth_npts=10,
        ...
        local_png = os.path.join(save_dir, "mms_null_vars.png")
        tplot(null_vars, save_png=local_png, display=global_display)
    ...
    def test_find_magnetic_nulls_fote_cluster(self):
        data = pyspedas.projects.cluster.load_csa(
            probes=["C1", "C2", "C3", "C4"],
            trange=["2003-08-17/16:40", "2003-08-17/16:45"],
            datatypes="CP_FGM_FULL",
            time_clip=True,
        )
        self.assertEqual(len(data), 12)
        # tplot_names()
        fields = [
            "B_vec_xyz_gse__C" + prb + "_CP_FGM_FULL" for prb in ["1", "2", "3", "4"]
        ]
        positions = [
            "sc_pos_xyz_gse__C" + prb + "_CP_FGM_FULL" for prb in ["1", "2", "3", "4"]
        ]
        null_vars = pyspedas.find_magnetic_nulls_fote(
            fields=fields, positions=positions, smooth_fields=True
        )
        local_png = os.path.join(save_dir, "cluster_null_vars.png")
        tplot(null_vars, display=global_display, save_png=local_png)
        d = get_data("max_reconstruction_error")
        np.testing.assert_allclose(d.y, 0.0, atol=1.0e-10)
