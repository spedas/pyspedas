...
def tinterpol_mxn(source, target, *, method='linear', newname=None,
    ...
    --------
    >>> import pyspedas
    >>> result = pyspedas.tinterpol_mxn({'x': [0, 2], 'y': [0, 4]}, [0, 1, 2])
    >>> result['y']
    array([0., 2., 4.])
    """
    ...
    if method != 'linear':
        raise ValueError("tinterpol_mxn currently supports only method='linear'.")
    ...
    for name, result in zip(output_names, results):
        if not result.sizes['time']:
            logging.info("tinterpol_mxn: empty result for %s; skipping storage", name)
            continue
