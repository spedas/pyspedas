...
class CotransTestCases(unittest.TestCase):
    ...
    def test_get_set_coords(self):
        ...
        with self.assertLogs(level="ERROR") as log:
            ...
            self.assertIn(
                "cotrans error: No input coordinates were provided.", log.output[0]
            )
    ...
    def test_dsl2gse(self):
        ...
        pyspedas.projects.themis.fgm(
            probe="a", trange=time_range, varnames=["tha_fgl_dsl"]
        )
        ...
        dsl2gse("tha_fgl_dsl", "tha_fgl_gse", isgsetodsl=True)
    ...
    def test_cotrans(self):
        ...
        pyspedas.projects.themis.state(
            probe=probe, trange=trange, time_clip=True, varnames=[name_in]
        )
    ...
    def test_cotrans_coord_mismatch(self):
        ...
        with self.assertLogs(level="ERROR"):
            result = cotrans(
                name_in=name_in, name_out=name_out, coord_in="gsm", coord_out="geo"
            )
    ...
    def test_cotrans_igrf(self):
        ...
        name_out = "tname_out"
        cotrans(
            name_out=name_out, time_in=t, data_in=d, coord_in="gse", coord_out="gsm"
        )
        in_len = len(t)
        dout = get_data(name_out)
        out_len = len(dout[0])
    ...
    def test_cotrans_j2000(self):
        ...
        name_out = "tname_out"
        cotrans(
            name_out=name_out, time_in=t, data_in=d, coord_in="gei", coord_out="j2000"
        )
        in_len = len(t)
        dout = get_data(name_out)
        out_len = len(dout[0])
    ...
    def test_cotrans_geigsm(self):
        ...
        name_out = "tname_out"
        cotrans(
            name_out=name_out, time_in=t, data_in=d, coord_in="gei", coord_out="gsm"
        )
        in_len = len(t)
        dout = get_data(name_out)
        out_len = len(dout[0])
    ...
    def test_all_cotrans(self):
        ...
        with self.assertLogs(level="ERROR") as log:
            ...
            self.assertTrue(result == 0)
            self.assertIn(
                "cotrans error: Requested input coordinate system", log.output[0]
            )
            result = cotrans(
                name_out=name1,
                time_in=t,
            ...
            self.assertTrue(result == 0)
            self.assertIn(
                "cotrans error: Requested output coordinate system", log.output[1]
            )

        # Test empty data.
        with self.assertLogs(level="ERROR") as log:
            cotrans(
                name_out=name1, time_in=t, data_in=[], coord_in="gei", coord_out="geo"
            )
    ...
    def test_mlt(self):
        ...
        sm_idl = [
            [33199.660, 25815.376, 2995.6487],
            [18814.747, 37614.243, 2990.7136],
            [-40428.563, -11607.452, 2995.3227],
            [15798.372, -38974.509, 2995.0028],
        ]
    ...
    def test_contrans_single_point(self):
        """Test cotrans with a single data point input."""
        data_in = np.array([[3.46410162, 3.46410162, 3.46410162]])
        time_in = [time_double("2015-12-30T00:00:00")]
        known_frames = ["GSE", "GSM", "GEI", "SM", "GEO", "J2000"]
        for f1 in known_frames:
            for f2 in known_frames:
                res = cotrans(
                    time_in=time_in, data_in=data_in, coord_in=f1, coord_out=f2
                )
