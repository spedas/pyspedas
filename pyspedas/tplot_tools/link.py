...
def link(names, link_name, link_type='alt'):
    ...
    for i in names:
        if i not in pyspedas.tplot_tools.data_quants.keys():
            logging.info(str(i) + " is not currently in pyspedas.")
            return
        
        if isinstance(i,int):
            i = list(pyspedas.tplot_tools.data_quants.keys())[i-1]

        if isinstance(pyspedas.tplot_tools.data_quants[i], dict):  # non-record varying variable
            continue
        pyspedas.tplot_tools.data_quants[pyspedas.tplot_tools.data_quants[i].name].attrs['plot_options']['links'][link_type] = link_name
