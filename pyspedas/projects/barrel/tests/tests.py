...
class LoadTestCases(unittest.TestCase):
    ...
    def test_sspc(self):
        del_data("*")
        sspc_vars = pyspedas.barrel.sspc(probe='1A')
    ...
    def test_sspc_time_clip(self):
        ...
        sspc_vars = pyspedas.barrel.sspc(probe='1A',trange=trange,time_clip=True)
        self.assertTrue(len(sspc_vars) > 0)
        self.assertTrue('brl1A_SSPC' in sspc_vars)
        self.assertTrue(data_exists('brl1A_SSPC'))
    ...
    def test_mspc(self):
        del_data("*")
        mspc_vars = pyspedas.barrel.mspc(probe='1A')
    ...
    def test_fspc(self):
        del_data("*")
        fspc_vars = pyspedas.barrel.fspc(probe='1A')
    ...
    def test_rcnt(self):
        del_data("*")
        rcnt_vars = pyspedas.barrel.rcnt(probe='1A')
    ...
    def test_magn(self):
        del_data("*")
        magn_vars = pyspedas.barrel.magn(probe='1A')
    ...
    def test_hkpg(self):
        del_data("*")
        hkpg_vars = pyspedas.barrel.hkpg(probe='1A')
