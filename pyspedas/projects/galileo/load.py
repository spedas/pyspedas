"""Load Galileo data using the DAS2 download and parsing utilities."""

from pyspedas import time_float_one
from pyspedas.projects.galileo.config import CONFIG
...
def get_info(datatype=""):
    """Get information about a Galileo dataset from the University of Iowa DAS2 server.
...
def load(
    ...
    """Load Galileo data from the University of Iowa DAS2 server.
    ...
    datatype : str, required
        Dataset abbreviation or full DAS2 dataset path from CONFIG["datasets"].
        Abbreviations are case-insensitive; full paths are case-sensitive.
        For example, "magnitude" and "Galileo/MAG/Magnitude" select the same
    ...
    >>> variables = load(
    ...     trange=["1997-02-02T12:00:00", "1997-02-02T14:00:00"],
    ...     datatype="magnitude",
    ...     interval=300,
    ... )
    """
    ...
    if dataset is None:
        ...
        if dataset["abrev"] == "magnitude" or dataset["abrev"] == "fce":
            # For magnitude and fce data, do not set the interval
            interval = None

    # Request ASCII records using the full path, regardless of which selector
    # the caller supplied (an abbreviation or the full dataset path).
    data = das2ascii(
        url=url,
        server="dataset",
        dataset=dataset["full_set"],
    ...
    # Default to the dataset abbreviation only when no prefix was supplied.
    # Forward the suffix unchanged so it follows each output variable name.
    abbr = prefix or "galileo_"
    result = das2tplot(
        data,
        prefix=abbr,
        suffix=suffix,
