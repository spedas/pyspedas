...
def make_databar_sroi_tvar(unix_starts,unix_ends, suffix:str = ''):
    """
    ...
    -------
    str
        Returns name of tplot variable created.
    """
    ...
    varname = 'mms_bss_sroi' + suffix
    vars_created = store_data(varname, data={'x': bar_x, 'y': bar_y})

    if not vars_created:
        logging.error(f'Error creating SROI fast survey segment intervals tplot variable {varname}')
        return
    ...
    options(varname, 'legend_names', ['Fast (SROI)'])
