...
from pyspedas.tplot_tools import data_exists, get_data, tplot, tplot_names
...
class FPITestCases(unittest.TestCase):
    ...
    def test_qflux(self):
        # Compare the heat flux moments generated by moments_3d to those generated by the MMS SDC
        trange = ['2015-11-19/08:34:41', '2015-11-19/08:35:53']
        mms_load_fpi(trange=trange,probe='1',datatype='des-moms',data_rate='brst',time_clip=True)
        mms_part_getspec(trange=trange,probe='1',species='e',data_rate='brst',output='moments',sdc_units=True)
        pyspedas.time_clip('*qflux','2015-11-19/08:34:50','2015-11-19/08:35:00',suffix='-tclip')
        pyspedas.time_clip('*heatq*', '2015-11-19/08:34:50', '2015-11-19/08:35:00',suffix='-tclip')
        d1=pyspedas.get_data('mms1_des_dist_brst_qflux-tclip')
        d2=pyspedas.get_data('mms1_des_heatq_dbcs_brst-tclip')
        d3=pyspedas.get_data('mms1_des_heatq_err_brst-tclip')
        pgs_norm_data = d1.y/d3.y
        mms_norm_data = d2.y/d3.y
        pyspedas.store_data('pgs_norm_tclip',data={'x':d1.times,'y':pgs_norm_data})
        pyspedas.store_data('mms_norm_tclip',data={'x':d1.times,'y':mms_norm_data})
        pyspedas.subtract('mms1_des_dist_brst_qflux-tclip','mms1_des_heatq_dbcs_brst-tclip',newname='diff_tclip')
        tplot('*tclip',display=global_display, save_png='mms1_fpi_heatflux.png')
        assert_allclose(d1.times,d2.times)
        # Using uncorrected e values in moments_3d, max difference is 0.16984655 absolute
        # When using e_inf to account for spacecraft potential, max difference is 0.16962053 absolute
        assert_allclose(d1.y, d2.y,atol=0.17)
