import logging
import numpy as np
from pyspedas.pytplot import time_string, time_double, tnames
import pyspedas
...
def mms_feeps_remove_bad_data(probe='1', data_rate='srvy', datatype='electron', level='l2', suffix='', trange=None):
    ...
    for bad_var in bad_vars:
        if not bad_var:
            continue
        bad_var_data = pyspedas.pytplot.get(bad_var[0])

        if bad_var_data is not None:
            times, data, energies = bad_var_data
            data[:] = np.nan
            pyspedas.pytplot.store(bad_var[0], data={'x': times, 'y': data, 'v': energies})
