...
class Dscovr_LoadTestCases(unittest.TestCase):
    ...
    def test_load_pre_at_data(self):
        del_data()
        pre_at_vars = pyspedas.dscovr.load(
    ...
    def test_load_invalid_trange(self):
        del_data()
        t_all = pyspedas.dscovr.all(trange="bad input")
        self.assertTrue(len(t_all) == 0)
        del_data()
        t_all = pyspedas.dscovr.all(trange=["2018-12-15"])
        self.assertTrue(len(t_all) == 0)
        del_data()
        t_all = pyspedas.dscovr.all(trange=["2018-12-15", "2018-12-14"])
        self.assertTrue(len(t_all) == 0)

    def test_load_invalid_instrument(self):
        del_data()
        t_all = pyspedas.dscovr.all(instrument="bad input")
        self.assertTrue(len(t_all) == 0)
