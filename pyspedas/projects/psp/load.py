...
def load(trange=['2018-11-5', '2018-11-6'],
    ...
    if instrument == 'fields':
        ...
        if datatype in ['mag_rtn', 'mag_sc']:
            ...
            return out_vars
        elif datatype[:12] == 'dfb_dc_spec_' or datatype[:12] == 'dfb_ac_spec_' or datatype[
            :13] == 'dfb_dc_xspec_' or datatype[:13] == 'dfb_ac_xspec_':
    ...
    if username is None:
        ...
        if instrument == 'fields':
            ...
            try:
                ...
                if len(files) != len(
                        remote_dates):  # I think this is a temp-fix, the logic of these blocks doesnt work now that download() doesnt raise an exception
                    if str(type(trange[
                                    1])) == "<class 'datetime.datetime'>":  # Handle case when user input is type <datetime.datetime>
